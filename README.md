...
<a id="dashboard"></a>
### Dashboard
- **5 contatori cliccabili *(v1.14.0)*** in tempo reale: clienti registrati, bici in officina, pronte al ritiro (evidenziato in verde), consegnate oggi, consegnate non pagate. Ogni card è navigabile anche da tastiera (`role="button"`) e apre la vista corrispondente con il **filtro ordini già impostato** (nuovo filtro `🔧 In officina (non consegnati)` per la card "In officina")
...
- **↩️ Undo avanzamento stato *(v1.14.0)*** — dopo aver cliccato "🔧 Inizia / ✅ Pronto / 📦 Consegna" un toast con azione "↩️ Annulla" resta visibile 6 secondi e permette di tornare allo stato precedente (rispettando scarico/carico magazzino)
...
### 🔍 Ricerca Globale *(v1.7.0 · v1.14.0)*
- Barra di ricerca nell'header, accessibile da qualsiasi vista
- **Scorciatoia `/`** *(v1.14.0)* — premi `/` fuori dai campi input per portare il focus alla ricerca globale. Sostituisce di fatto `Ctrl+F`, che su Chrome/Firefox/Edge è riservato dal browser per il "Trova nella pagina" e non può essere sovrascritto da una pagina web
...
- **📱 Form ad accordion su mobile *(v1.14.0)*** — sotto i 768px il modal ordine si trasforma in 5 sezioni collassabili (`👤 Dati` · `🔧 Lavorazioni` · `📦 Ricambi` · `💬 Timeline` · `💰 Pagamento`) con **badge riassuntiva** su ogni header che si aggiorna in tempo reale (`3 voci · € 45,00`, `⚠ cliente mancante`, `✓ senza bici`, `Anticipo € 20,00`); **totale ordine + Salva/Annulla sticky** in fondo, sempre visibili durante lo scroll. Su desktop il layout resta a scroll unico. **Validazione contestuale:** se manca il cliente o le lavorazioni, l'accordion apre automaticamente la sezione col problema e mette il focus sul campo mancante
- **💬 Auto-apertura Timeline *(v1.14.0)*** — il badge commenti `💬 N` sulle card ordini è ora cliccabile e apre l'ordine direttamente sulla sezione Timeline. Il meccanismo è esteso via `apriModalOrdine(id, preClienteId, { stepOpen })` per aprire l'ordine su una sezione specifica
...
- **4 stati** con avanzamento sequenziale tramite pulsante dedicato — con **↩️ Undo *(v1.14.0)*** via toast "Annulla" per 6 secondi che riporta all'ordine allo stato precedente (magazzino sincronizzato di conseguenza)
...
| **Form ordine step / accordion mobile** | — | ✅ Implementato in v1.14.0 (accordion mobile con badge, totale sticky, validazione contestuale) |
| **Accessibilità modali (focus trap, aria)** | — | ✅ Implementato in v1.14.0 |
| **Undo azioni rapide (avanzamento stato)** | — | ✅ Implementato in v1.14.0 (toast "Annulla" 6s) |
| **Notifiche pronto-ritiro** | 🟡 Media | Pulsante “Notifica WhatsApp” (gratuito, link `wa.me`) o SMS/email via API Twilio/nodemailer |
| **Promemoria manutenzione periodica** | 🟡 Media | Lista clienti da richiamare in dashboard (tagliando +6 mesi dopo la consegna) per fidelizzazione |
| **QR/etichetta bici** | 🟡 Media | QR code stampabile che apre lo storico interventi della bici — utile in officina con più bici in coda |
| **Preventivo con approvazione cliente** | 🟡 Media | Preventivo pre-lavorazione con link/PDF di approvazione (riduce contestazioni sul prezzo finale) |
| **Agenda/appuntamenti** | 🟡 Media | Calendario ritiro/consegna per evitare accavallamenti nei giorni di punta |
...
|---|---|---|
| **1.14.0** | 2026-07-28 | **Passata di miglioramento UX/accessibilità (nessuna modifica al DB o alla logica di dominio).** **📱 Form ordine → accordion su mobile:** sotto i 768px il modal ordine si trasforma in un accordion a 5 sezioni collassabili (`👤 Dati` · `🔧 Lavorazioni` · `📦 Ricambi` · `💬 Timeline` · `💰 Pagamento`) con badge riassuntiva su ciascun header (`3 voci · € 45,00`, `⚠ cliente mancante`, `✓ senza bici`, `Anticipo € 20,00`, ecc.) aggiornata in tempo reale; **totale ordine + Salva/Annulla sticky** in fondo, sempre visibili durante lo scroll di una sezione. Su desktop il layout resta a scroll unico (nessuna regressione). **Validazione contestuale:** al submit, se manca il cliente o le lavorazioni, l'accordion apre automaticamente la sezione col problema e mette il focus sul campo mancante (invece del solo toast rosso). **Auto-apertura contestuale:** il badge commenti `💬 N` sulle card ordini è ora un bottone cliccabile che apre l'ordine direttamente sulla sezione Timeline; il meccanismo è generalizzato tramite un nuovo parametro `apriModalOrdine(id, preClienteId, { stepOpen })`. **↩️ Undo per l'avanzamento rapido di stato:** il click su "🔧 Inizia / ✅ Pronto / 📦 Consegna" mostra un toast con azione "Annulla" per 6 secondi che ripristina lo stato precedente passando dalla PUT server-side (rispetta scarico/carico magazzino, con toast di feedback dedicato); nuova funzione `OrdiniService.impostaStato(id, stato)`. **📊 Card dashboard cliccabili + nuovo filtro "In officina":** le 5 stat-card (`Clienti · In officina · Pronte al ritiro · Consegnate oggi · Consegnate non pagate`) sono ora `role="button"` navigabili anche da tastiera, portano direttamente alla vista corrispondente con il filtro ordini già impostato (nuova opzione `🔧 In officina (non consegnati)` nel select filtri). **⌨️ Scorciatoia `/` per la ricerca globale:** `Ctrl+F` era intercettato dal browser (Chrome/Firefox/Edge non permettono a una pagina di sovrascriverla, quindi apriva il "Trova nella pagina" nativo); ora premendo `/` (fuori dai campi input) il focus va sempre alla ricerca globale. Il modal scorciatoie riporta il nuovo tasto + una nota su Windows/Linux. Placeholder ricerca aggiornato a "🔍 Cerca ovunque… (premi /)". **🍞 Toast in coda con `aria-live`:** un contenitore unico `#toast-container` impila i toast in colonna invece di sovrapporli quando ne arrivano più ravvicinati (es. "Ordine salvato" + `_magazzino.warnings`); `role=alert` per gli errori, `role=status` per i successi; supporto opzionale di un pulsante azione (usato per l'undo). **🎯 Accessibilità dei modali:** ogni modal riceve automaticamente `role="dialog"`, `aria-modal="true"` e `aria-labelledby` collegato al titolo; **focus-trap sul Tab** (Tab/Shift+Tab wrappano nel modale in primo piano invece di uscire dietro l'overlay); il focus torna sull'elemento che ha aperto il modale alla chiusura (WCAG 2.4.3). **📐 Header responsive:** i pulsanti `🌙` tema e `⌨️` scorciatoie sono ora raggruppati in `.header-actions` così restano sempre affiancati; sotto i 760px l'header si riorganizza in modo prevedibile (Riga 1: logo + azioni tema/scorciatoie · Riga 2: ricerca a tutta larghezza · Riga 3: nav scorrevole) invece di spezzarsi in modo imprevedibile. **File modificati:** `public/index.html`, `public/css/style.css`, `public/js/app.js`, `public/js/ui.js`, `public/js/ordini.js` — nessuna modifica al backend, nessuna migrazione DB. |
...
*🚲 CicloDesk v1.14.0 — Gestionale per ciclo officina Cerica Bikelab*
